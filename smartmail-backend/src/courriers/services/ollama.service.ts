...
export class OllamaService {
  ...
  private buildPrompt(text: string): string {
    return [
      ...
      'Le courrier peut être en FRANÇAIS ou en ARABE. Extrais les champs quelle que soit la langue.',
      '',
      'Champs attendus :',
      ...
      `- serviceCode : le service destinataire le plus adapté pour traiter ce courrier, parmi ${VALID_SERVICES.join(', ')}.`,
      '  Règle ABSOLUE pour serviceCode (ne jamais enfreindre) :',
      '    SI le courrier parle de recrutement, candidature, offre d\'emploi, embauche, poste à pourvoir, CV, stage, recrutement de personnel',
      '    ALORS serviceCode = "RH" (même si le poste est technique comme technicien ou ingénieur).',
      '    SINON :',
      '      - pannes, incidents, fibre, ADSL, internet, connexion, réseau, messagerie, installation, raccordement => TECHNIQUE',
      '      - factures, paiements, remboursement, budget, comptabilité, frais, impayés => FINANCE',
      '      - offres, abonnements, contrats, résiliations, partenariats, devis, tarifs, ventes, clients => COMMERCIAL',
      '      - congés, salaire, personnel, accident de travail, arrêt maladie => RH',
      '  Règles équivalentes en ARABE pour serviceCode :',
      '    توظيف/انتداب/منصب شغل/مترشح/مطلب شغل => RH',
      '    عطب/انقطاع/خلل/شبكة/اتصال/إنترنت => TECHNIQUE',
      '    فاتورة/تسديد/دفع/استرجاع/محاسبة => FINANCE',
      '    عرض/اشتراك/عقد/إلغاء/شراكة/زبون/تجاري => COMMERCIAL',
      '',
      '  Exemple : "Candidature spontanée au poste de technicien réseau" => serviceCode = "RH" (car candidature, même si le poste est technique).',
      '  Exemple : "مطلب انتداب تقني سامي في الشبكة" => serviceCode = "RH" (انتداب رغم أن المنصب تقني).',
      '',
      'COURRIER :',
      '"""',
      text,
